...
hdr = (r"Session & EDA OOR & EDA flat & SCR flat & HR/IBI drop & IBI OOR & Gap (s) & EDA ok & HR ok \\")

lines = [
    r"\footnotesize",
    r"\setlength{\tabcolsep}{4pt}",
    r"\begin{longtable}{@{}lrrrrrrcc@{}}",
    ...
    r"out-of-range (\%) among present rows; Gap = longest HR/IBI dropout (s); EDA ok / HR ok = passes the session-level "
    r"screen. The table covers the four biometric channels that show any non-trivial "
...
for tag, r in tbl.iterrows():
    cells = [esc(tag), f"{r.eda_oor:.1f}", f"{r.eda_flat:.1f}", f"{r.scr_flat:.1f}",
             f"{r.hr_drop:.1f}", f"{r.ibi_oor:.1f}", f"{r.hr_gap:.1f}", ck(r.eda_ok), ck(r.hr_ok)]
...
print(f"Sessions failing EDA screen (flatline >= {FLAT_SCREEN:.0f}%): {(~tbl.eda_ok).sum()} "
      f"(drivers {sorted(tbl[~tbl.eda_ok].driver.unique())})")
print(f"Sessions failing HR/IBI screen (dropout >= {DROPOUT_SCREEN:.0f}%): {(~tbl.hr_ok).sum()} "
      f"(drivers {sorted(tbl[~tbl.hr_ok].driver.unique())})")
print(f"Sessions with SCR_FREQ flatline >= {FLAT_SCREEN:.0f}%: {(tbl.scr_flat >= FLAT_SCREEN).sum()}")
print(f"Sessions passing both screens: {(tbl.eda_ok & tbl.hr_ok).sum()} / {len(tbl)}")
